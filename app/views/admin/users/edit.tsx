    ...
    <summary class="d-flex align-items-center gap-3">
      ...
      <div>
        ...
        <p class="form-text mb-0">
          {entry.authorizedAt ? "Authorized at " : "Created at "}
          <Time
            unix={entry.authorizedAt ?? entry.createdAt!}
            dateStyle="long"
            timeStyle="short"
          />
          <br />
          {entry.owner ? (
            <>
              <i class="bi bi-person-fill text-primary" /> Owned by{" "}
            ...
            <>
              <i class="bi bi-shield-fill-check text-success" /> System
            </>
          )}
        </p>
      </div>
...
const TokenEntry = ({ token }: { token: EditPage_Token }) => (
  <details class="mb-3 border rounded p-3">
    <summary class="d-flex align-items-center gap-3">
      <i class={`token-icon bi bi-key${token.authorizedAt ? "-fill" : ""}`} />
      <div>
        <h6 class="mb-1">{token.name ?? ""}</h6>
        <p class="form-text mb-0">
          {token.authorizedAt ? "Updated at " : "Created at "}
          <Time
            unix={token.authorizedAt ?? token.createdAt}
            dateStyle="long"
            timeStyle="short"
          />
        </p>
...
mountProtoPage(
  ...
  ({
    ...
    const baseRoles = useRef<Role[]>([...initialAccount.roles])
